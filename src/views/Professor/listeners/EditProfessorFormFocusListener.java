...
public class EditProfessorFormFocusListener implements FocusListener {
	...
	public void focusLost(FocusEvent e) {
		...
		if (result && input.getName().compareTo("email") == 0) {
			if (!(EditProfessorDialog.getInstance().getEditForm().getCurrentEmail().compareTo(input.getText()) == 0)) {
			...
			if (!(EditProfessorDialog.getInstance().getEditForm().getCurrentIdCardNumber() == Integer.parseInt(input.getText()))) {
